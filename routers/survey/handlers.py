...
async def handle_survey_user_full_name_invalid_content_type(message: types.Message):
    await message.answer(
        f"Sorry, I didn't understand, send your full name as text.",
    )


@router.message(Survey.email, valid_email_filter)
async def handle_survey_user_email(
    message: types.Message, state: FSMContext, email: str
):
    await state.update_data(email=message.text)
    await state.set_state(Survey.email_news_letter)
    await message.answer(
        text=f"Cool, your email is now {markdown.hcode(email)}.\n"
             f"Would you like to be contacted in future?",
        reply_markup=build_yes_or_no_kb(),
    )


@router.message(Survey.email)
async def handle_survey_invalid_user_email(
...
@router.message(Survey.email_news_letter, F.text.casefold() == "yes")
...
@router.message(Survey.email_news_letter, F.text.casefold() == "no")
