...
from keyboards.inline_keyboards.shop_kb import ShopCbData, ShopActions, build_shop_kb, build_products_kb, ProductCbData, \
    ProductActions, product_details_kb, build_update_product_kb

router = Router(name=__name__)


@router.callback_query(
    ShopCbData.filter(F.actions == ShopActions.products)
)
...
@router.callback_query(
    ShopCbData.filter(F.actions == ShopActions.address)
)
...
@router.callback_query(
    ShopCbData.filter(F.actions == ShopActions.root)
)
async def handle_my_address_button(call: CallbackQuery):
    await call.answer()
    await call.message.edit_text(
